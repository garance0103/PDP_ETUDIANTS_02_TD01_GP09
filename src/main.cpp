...
const char* wifi_ssid = "samsungval";
...
void setup() {
  ...
  if (!isnan(event.temperature)) {
    mqtt_client.publish("TD02_GP09/temp", String(event.temperature).c_str());
  }

  dht.humidity().getEvent(&event);
  if (!isnan(event.relative_humidity)) {
    mqtt_client.publish("TD02_GP09/relhum", String(event.relative_humidity).c_str());
  }
